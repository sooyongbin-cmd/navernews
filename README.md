...
검색어로 네이버 뉴스 최신 결과 3건을 조회하고, 세 언론사 원문의 읽기 본문을 모두 확보했을 때만 종합 브리핑을 생성하는 Next.js 앱입니다. 기존 Vercel AI Gateway 브리핑과 Google Gemini API 직접 호출 브리핑을 각각 선택해서 사용할 수 있습니다.

## 주요 동작

1. `/api/search`가 네이버 뉴스 검색 API를 `display=3`, `sort=date`로 호출합니다.
2. 검색된 세 원문 URL을 5분짜리 HMAC 서명 토큰으로 묶습니다.
3. 사용자가 버튼을 누르면 기존 `/api/briefing` 또는 별도의 `/api/gemini-briefing`이 실행됩니다.
4. 서버가 `robots.txt`를 확인하고 세 원문을 병렬로 가져옵니다.
5. `Mozilla Readability`로 세 기사 전문이 모두 추출된 경우에만 선택한 모델을 한 번 호출합니다.
