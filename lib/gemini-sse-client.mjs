import { validateGeminiInfographic } from "./gemini-infographic.mjs";
...
export async function consumeGeminiSse(
  response,
  { onSearch, onDelta, onInfographic } = {}
) {
  if (!response.body) {
    throw new GeminiSseError({
  ...
  async function handleFrame(frame) {
    ...
    if (parsed.eventName === "search") {
      const items = parsed.data?.items;
      if (
        !Array.isArray(items) ||
        items.length > 3 ||
        items.some((item) => item && Object.hasOwn(item, "text"))
      ) {
        throw new GeminiSseError({
          code: "GEMINI_SEARCH_PROTOCOL_ERROR",
          message: "자동 검색 결과의 응답 형식이 올바르지 않습니다.",
        });
      }
      search = parsed.data;
    ...
    if (parsed.eventName === "infographic") {
      try {
        infographic = validateGeminiInfographic(parsed.data?.infographic);
        await onInfographic?.(infographic);
      } catch {
        throw new GeminiSseError({
          code: "GEMINI_INFOGRAPHIC_PROTOCOL_ERROR",
          message: "SVG 인포그래픽 데이터의 응답 형식이 올바르지 않습니다.",
        });
      }
      return;
    }
  ...
  if (!completion?.skipped && !infographic) {
    throw new GeminiSseError({
      code: "GEMINI_INFOGRAPHIC_MISSING",
      message: "Gemini 브리핑에 SVG 인포그래픽 데이터가 포함되지 않았습니다.",
    });
  }

  return { ...completion, search, infographic };
}
